...
object RunSettings {
  private[RunSettings] def decodeScalaName(name: String): String = {
    try {
      val cl = Class.forName("scala.reflect.NameTransformer") // TODO remove reflection
      val m = cl.getMethod("decode", classOf[String])
      val decoded = m.invoke(null, name).asInstanceOf[String]
      if (decoded == null) name
      else decoded
    } catch {
      case t: Throwable => name
    }
  }
}